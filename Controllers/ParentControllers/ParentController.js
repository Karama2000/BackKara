...
exports.getChildrenProgress = async (req, res) => {
  try {
    ...
    lessonsProgress.forEach((p) => {
      if (!p.lessonId) {
        console.warn(`Lesson progress ${p._id} has no valid lessonId reference`);
      }
    });
    quizSubmissions.forEach((q) => {
      if (!q.quizId) {
        console.warn(`Quiz submission ${q._id} has no valid quizId reference`);
      }
    });
    testSubmissions.forEach((t) => {
      if (!t.testId) {
        console.warn(`Test submission ${t._id} has no valid testId reference`);
      }
    });
